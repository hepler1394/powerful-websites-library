(function () {
  ...
  root.innerHTML = [
    ...
    '<header class="pw-header">',
      ...
      '<nav class="pw-nav" aria-label="Primary"><a href="#pw-directory">Directory</a><button id="pw-motion" type="button" aria-pressed="false">Motion on</button><button id="pw-browse" type="button">Browse cards</button><button id="pw-saved" type="button">Saved <span class="pw-nav__count" id="pw-saved-count">0</span></button></nav>',
    '</header>',
    ...
    '<main class="pw-discovery" id="pw-discover">',
      '<section class="pw-directory pw-directory--primary pw-reveal" id="pw-directory" aria-labelledby="pw-directory-title"><header class="pw-directory__head"><div><p class="pw-section-code">THE COMPLETE DIRECTORY</p><h2 id="pw-directory-title">Find a site.<br><em>Open a card.</em></h2></div><p>Choose a category above or below. Each choice takes you straight to its cards — no extra menu.</p></header><nav class="pw-jump" id="pw-jump" aria-label="Jump to a website category"></nav><div class="pw-directory__groups" id="pw-directory-groups"></div></section>',
      '<footer class="pw-footer"><a class="pw-brand" href="#pw-top"><span class="pw-brand__pulse"></span><span class="pw-brand__name">Powerful Websites</span></a><p>Useful websites, filed by humans. Saved links stay in this browser.</p><div><a href="faq.html">Manifesto</a><a href="#pw-top">Top ↑</a></div></footer>',
    '</main>',
  ...
  function showCategory(category, sourceButton, options) {
    closePanel({ keepUrl: true });
    document.querySelectorAll('.pw-category').forEach(function (button) { button.removeAttribute('aria-current'); });
    if (sourceButton) sourceButton.setAttribute('aria-current', 'true');
    visual.focusGroup = categories.indexOf(category); visual.searchRatio = 0; visual.burst = 1;
    var shelf = document.getElementById('pw-shelf-' + category.id);
    if (shelf) shelf.scrollIntoView({ behavior: reduceMotion ? 'auto' : 'smooth', block: 'start' });
    if (!options || !options.skipHistory) history.replaceState(null, '', '#pw-shelf-' + category.id);
  }
  ...
  function showBrowse() { closePanel({ keepUrl: true }); var directory = document.getElementById('pw-directory'); if (directory) directory.scrollIntoView({ behavior: reduceMotion ? 'auto' : 'smooth', block: 'start' }); history.replaceState(null, '', '#pw-directory'); }
  ...
  function renderDiscovery() {
    ...
    categories.forEach(function (category, categoryIndex) {
      ...
      function drawShelf() { var fragment = document.createDocumentFragment(); (expanded ? sites : sites.slice(0, 6)).forEach(function (site, index) { fragment.appendChild(createSiteCard(site, index)); }); cards.replaceChildren(fragment); open.textContent = expanded ? 'Every card shown' : 'Show every card ↓'; open.disabled = expanded; toggle.textContent = expanded ? 'Show fewer cards ↑' : 'Show all ' + sites.length + ' websites ↓'; toggle.setAttribute('aria-expanded', String(expanded)); }
  ...
  document.getElementById('pw-random').addEventListener('click', showRandomSignal); var randomBottom = document.getElementById('pw-random-bottom'); if (randomBottom) randomBottom.addEventListener('click', showRandomSignal);
  ...
  if ('IntersectionObserver' in window) {
    ...
    var intro = document.querySelector('.pw-intro'); if (intro) { var typeObserver = new IntersectionObserver(function (entries) { entries.forEach(function (entry) { if (entry.isIntersecting) { typeIntro(); typeObserver.disconnect(); } }); }, { threshold: .25 }); typeObserver.observe(intro); }
